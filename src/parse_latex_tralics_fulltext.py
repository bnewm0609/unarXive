...
ARXIV_URL_PATT = re.compile(
    (r"arxiv\.org\/[a-z0-9]{1,10}\/(([a-z0-9-]{1,15}\/" r")?[\d\.]{5,10}(v\d)?$)"), re.I
)
ARXIV_ID_PATT = re.compile(r"^([a-zA-Z-\.]+)?\/?(\d\d)(\d\d)(.*)$")
ARXIV_ID_CITE_PATT = re.compile(r"arXiv:([\d\.]{5,10}(v\d)?)")
...
def _process_section_node(sec_node, curr_sec):
    ...
    text_elemnts = []
    skip_tags = ["clearpage", "newpage", "tableofcontents", "vfill"]
    for child_node in sec_node.getchildren():
        if child_node.tag in skip_tags:
            continue
        elif child_node.tag == "head":
            # head
            curr_sec = _process_section_head(sec_node, child_node)
        elif child_node.tag[:3] == "div":
            text_elemnts.extend(_process_section_node(child_node, curr_sec))
        else:
...
def _process_content_node(c_node, curr_sec):
    par_text = etree.tostring(c_node, encoding="unicode", method="text")
    cite_spans, ref_spans = _get_local_refs(par_text)
...
def _get_local_refs(par_text):
    marker_patt = re.compile(r"{{(cite|formula|figure|table|float):([0-9a-z-]+)}}")
    cite_spans = []
    ref_spans = []
...
def parse(
    ...
    log(
        ("Citations: {} (not unique)\nUnmatched citations: {}" "").format(
            num_citations, num_citations_notfound
        )
    )
    return True
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print(
            (
                "usage: python3 parse_latex_tralics.py </path/to/in/dir> </path/to/metadata.sqlite> </path"
                "/to/out/dir>"
            )
        )
        sys.exit()
    ...
    tar_fn = ""
    source_file_info = {}
    ret = parse(in_dir, out_dir, tar_fn, source_file_info, meta_db, incremental=False)
    if not ret:
        sys.exit()
