            ...
            if tarfile.is_tarfile(path):
                with tempfile.TemporaryDirectory() as tmp_dir_path:
                    ...
                    if main_tex_path is None:
                        log(
                            (
                                "couldn't find main tex file in dump archive {}" ""
                            ).format(fn)
                        )
                        continue
                    # "identify" bbl file
                    # https://arxiv.org/help/submit_tex#bibtex
                    main_tex_fn = os.path.normpath(main_tex_path).split(os.sep)[-1]
                    ...
                    if os.path.isfile(os.path.join(tmp_dir_path, bbl_fn)):
                        ...
                        latexpand_args = ["latexpand", main_tex_path]
                        # main_tex_fn]
                    ...
                    err.flush()
                    subprocess.run(
                        latexpand_args, stdout=out, stderr=err, cwd=tmp_dir_path
                    )
                    out.close()
                    err.close()
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(
            (
                "usage: python3 nomalize_arxiv_dump.py </path/to/dump/dir> </pa"
                "th/to/out/dir>"
            )
        )
        sys.exit()
